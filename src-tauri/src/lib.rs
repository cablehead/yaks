...
use xs::store::{Store as XsStore, Frame, ReadOptions, FollowOption, ZERO_CONTEXT};
...
async fn append_event(
    ...
    let frame = Frame {
        ...
        hash,
        meta: request.meta.map(|m| serde_json::Value::Object(m.into_iter().collect())),
        ttl: None,
    };
...
async fn initialize_store(app: &AppHandle) -> Result<Store> {
    ...
    if !has_yak {
        ...
        let appended_yak = store_lock.append(yak_frame)
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![append_event, get_cas_content, log_message])
